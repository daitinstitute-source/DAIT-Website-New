/**
 * The lead pipeline, with no hosting platform baked in.
 *
 * The site has lived on both Cloudflare Pages and Vercel, and the two disagree on
 * everything except the work itself: where functions live, how a request arrives,
 * where env vars come from. So the work lives here and each host gets a thin
 * adapter (`api/leads.ts` for Vercel, `functions/api/leads.ts` for Cloudflare, plus
 * the dev bridge in astro.config.mjs). Moving hosts should never mean rewriting this.
 *
 * On each lead:
 *   1. Validate + honeypot spam check
 *   2. Notify the institute (transactional email to LEADS_TO_EMAIL)
 *   3. Upsert the contact into a Brevo list, which triggers the nurture automation
 *   4. Create a Brevo CRM deal linked to that contact, the card worked in Deals
 ...
 * Every downstream step is best-effort: a flaky provider never fails the visitor.
 * Field names in the payload are the CONTRACT, do not rename (see EnquiryForm.tsx).
 *
 * CONTACT IDENTITY: Brevo keys contacts by email, but our forms only require name +
 ...
 *
 * Required env:  BREVO_API_KEY, LEADS_TO_EMAIL, LEADS_FROM_EMAIL
 * Optional env:  LEADS_FROM_NAME, BREVO_LIST_ID, BREVO_PIPELINE_ID, BREVO_STAGE_ID,
 *                LEAD_WEBHOOK_URL
 */
...
/** What the pipeline reports back; each adapter turns this into its own response. */
...
const result = (body: LeadResult, status = 200): LeadResponse => ({ status, body });
...
/**
 * Run one lead through the whole pipeline. Adapters parse the request body and
 * render the response; everything in between happens here.
 */
export async function handleLead(lead: LeadPayload, env: Env): Promise<LeadResponse> {
  // Honeypot: bots fill "company". Pretend success so they do not retry.
  if (lead.company && lead.company.trim() !== "") return result({ ok: true });

  // Minimal validation, name + phone are the must-haves.
  if (!lead.name?.trim() || !lead.phone?.trim()) {
    return result({ ok: false, error: "Name and phone are required." }, 422);
  }

  // Fan out; never fail the user if a downstream is flaky.
  // The CRM pair runs sequentially, a deal needs its contact id to link to.
  ...
  if (!notified && !forwarded && !crm.contacted) {
    // Nothing delivered (no credentials, or every provider failed). Leave a trace.
    console.log("[leads] captured (no delivery channel succeeded):", JSON.stringify(lead));
  }

  return result({ ok: true, notified, contacted: crm.contacted, dealt: crm.dealt, forwarded });
}
